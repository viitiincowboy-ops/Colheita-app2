...
function AnimNum({value, style:s={}}) {
  ...
  useEffect(()=>{
    ...
    const step=now=>{
      ...
      const raw=start+(end-start)*ease;
      // Preserve decimals if the target value has them
      const cur = Number.isInteger(end) ? Math.round(raw) : Math.round(raw*10)/10;
      setDisp(cur);
      ref.current=cur;
  ...
  return <span style={s}>{disp.toLocaleString("pt-BR",{maximumFractionDigits:2})}</span>;
}
...
function Resumo({registros}) {
  ...
  const totalAno=useMemo(()=>regAno.reduce((s,r)=>s+r.hectares,0),[regAno]);
  const totalAll=useMemo(()=>registros.reduce((s,r)=>s+r.hectares,0),[registros]);
  // Safras = culturas únicas por ano (ex: soja+milho em 2025 = 2 safras em 2025)
  const totalSafras=useMemo(()=>{
    const pares=new Set(registros.map(r=>`${anoOf(r.data)}-${r.cultura}`));
    return pares.size;
  },[registros]);
  const totalAnos=useMemo(()=>new Set(registros.map(r=>anoOf(r.data))).size,[registros]);
  ...
  const pivoPorCultura=useMemo(()=>{
    const m={};
    for(const r of regAno){
      if(!m[r.cultura]) m[r.cultura]={};
      const key=`${nomeFaz(r.fazenda)} — ${r.pivo}`;
      m[r.cultura][key]=(m[r.cultura][key]||0)+r.hectares;
    }
    return m;
  },[regAno]);
  ...
  const [cultPivExp,setCultPivExp]=useState(null);
    ...
    <div style={{paddingBottom:32}}>
          ...
          {CULTURAS.filter(c=>porCultura[c]).map(c=>{
            ...
            return (
              <div key={c} style={{
                ...
                overflow:"hidden",gridColumn:exp?"1/-1":"auto",
                boxShadow:exp?`0 0 24px ${CULTURA_COLORS[c]}22`:"0 2px 8px rgba(0,0,0,0.3)",
                transition:"all 0.25s",
              }}>
                {/* Só o cabeçalho é clicável para abrir/fechar */}
                <div onClick={()=>setCultExpand(exp?null:c)} style={{cursor:"pointer",padding:"12px 10px",textAlign:"center"}}>
                  ...
                  <div style={{fontSize:8,color:"#1a2a1a",marginTop:4}}>
                    {exp?"▲ fechar":""}
                  </div>
                </div>
                {exp&&(
                  <div style={{borderTop:`1px solid ${CULTURA_COLORS[c]}22`,padding:"8px 12px 14px"}}>
                    <div style={{fontSize:8,letterSpacing:2,textTransform:"uppercase",color:"#3a6a3a",marginBottom:8}}>
                      Por fazenda
                    </div>
                    {Object.entries(fazPorCultura[c]||{}).sort((a,b)=>b[1]-a[1]).map(([faz,fha])=>{
                      const fazKey=`${c}__${faz}`;
                      const pivosDesteFaz=Object.entries(pivoPorCultura[c]||{})
                        .filter(([k])=>k.startsWith(nomeFaz(faz)))
                        .sort((a,b)=>b[1]-a[1]);
                      const isPivOpen=cultPivExp===fazKey;
                      return (
                        <div key={faz} style={{marginBottom:6}}>
                          {/* Linha da fazenda — toda clicável, sem seta */}
                          <div
                            onClick={()=>setCultPivExp(isPivOpen?null:fazKey)}
                            style={{
                              display:"flex",justifyContent:"space-between",alignItems:"center",
                              padding:"9px 10px",
                              background:`${CULTURA_COLORS[c]}0d`,
                              border:`1px solid ${CULTURA_COLORS[c]}${isPivOpen?"44":"1a"}`,
                              borderRadius: isPivOpen?"8px 8px 0 0":"8px",
                              cursor:"pointer",
                            }}
                          >
                            ...
                            <div style={{textAlign:"right"}}>
                              <div style={{fontSize:14,fontWeight:"bold",color:CULTURA_COLORS[c]}}>{fha.toLocaleString("pt-BR",{maximumFractionDigits:2})} ha</div>
                              <div style={{fontSize:8,color:"#2a4a2a"}}>{Math.round((fha/ha)*100)}%</div>
                            </div>
                          </div>
                          {/* Pivôs — aparecem ao clicar na fazenda */}
                          {isPivOpen&&(
                            <div style={{
                              background:"#060e04",
                              ...
                              {pivosDesteFaz.map(([pivoKey,pha])=>{
                                const pivoNome=pivoKey.split(" — ")[1]||pivoKey;
                                return (
                                  <div key={pivoKey} style={{
                                    display:"flex",justifyContent:"space-between",alignItems:"center",
                                    padding:"7px 12px",
                                    borderBottom:`1px solid #0f1a08`,
                                  }}>
                                    <div style={{display:"flex",alignItems:"center",gap:6}}>
                                      <Icons.Drop size={9} color={CULTURA_COLORS[c]}/>
                                      <span style={{fontSize:11,color:"#6a8a5a"}}>{pivoNome}</span>
                                    </div>
                                    <div style={{textAlign:"right"}}>
                                      <span style={{fontSize:13,fontWeight:"bold",color:CULTURA_COLORS[c]}}>{pha.toLocaleString("pt-BR",{maximumFractionDigits:2})} ha</span>
                                      <div style={{fontSize:8,color:"#2a4a2a"}}>{Math.round((pha/fha)*100)}% da fazenda</div>
                                    </div>
                                  </div>
                                );
                              })}
      ...
      <div style={{background:"linear-gradient(145deg,#0f1e0a,#080e06)",border:"1px solid #152010",borderRadius:14,padding:"12px",boxShadow:"0 4px 20px rgba(0,0,0,0.4)"}}>
        ...
        <div style={{fontSize:11,color:"#2a4a2a",marginBottom:12}}>
          Total acumulado: <span style={{color:"#5a8a4a",fontWeight:"bold"}}>{totalAll.toLocaleString("pt-BR",{maximumFractionDigits:2})} ha</span>
          <span style={{color:"#3a5a2a"}}> · {totalSafras} {totalSafras===1?"safra":"safras"} · {totalAnos} {totalAnos===1?"ano":"anos"}</span>
        </div>
...
// ─── CONFIG TAB ───────────────────────────────────────────────────────────────
function Config({registros, onImport}) {
  const [importError, setImportError] = useState(null);
  const [confirmClear, setConfirmClear] = useState(false);
  const fileRef = useRef(null);

  const totalHa = registros.reduce((s,r)=>s+r.hectares,0);
  const anos = [...new Set(registros.map(r=>anoOf(r.data)))].sort((a,b)=>b-a);
  ...
  const handleExport = () => {
    ...
    a.href     = url;
    a.download = `JHS_Colheita_backup_${new Date().toISOString().split("T")[0]}.json`;
  ...
  const handleFileChange = (e) => {
    ...
    reader.onload = (ev) => {
      try {
        ...
        // Accept both raw array or our export format
        const lista = Array.isArray(parsed) ? parsed : parsed.registros;
        if(!Array.isArray(lista)) throw new Error("Formato inválido");
        // Validate fields
        for(const r of lista) {
          if(!r.data||!r.fazenda||!r.cultura||r.hectares===undefined) throw new Error("Dados incompletos");
        }
        onImport(lista);
      } catch(err) {
  ...
  return (
    <div style={{paddingBottom:32}}>
      ...
      <div style={{
        ...
        <div style={{fontSize:9,letterSpacing:3,textTransform:"uppercase",color:"#5a9a4a",marginBottom:3}}>Configurações</div>
        <div style={{fontSize:18,fontWeight:"bold",color:"#d0e0a0"}}>Backup de Dados</div>
        <div style={{fontSize:12,color:"#4a6a3a",marginTop:4}}>
          {registros.length} registros · {totalHa.toLocaleString("pt-BR",{maximumFractionDigits:2})} ha · {anos.length} {anos.length===1?"ano":"anos"}
        </div>
      </div>
      ...
      <div style={{
        background:"linear-gradient(145deg,#0f1e0a,#080e06)",
        border:"1px solid #152010",borderRadius:14,padding:"16px",marginBottom:12,
        boxShadow:"0 4px 16px rgba(0,0,0,0.4)",
      }}>
        <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:10}}>
          <div style={{
            width:40,height:40,borderRadius:10,flexShrink:0,
            background:"linear-gradient(135deg,#1a4a0a,#0a2006)",
            border:"1px solid #2a6a1a",
            display:"flex",alignItems:"center",justifyContent:"center",
          }}>
            <svg width="20" height="20" viewBox="0 0 24 24" fill="none">
              <path d="M12 2v13M8 11l4 4 4-4" stroke="#6ab04a" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/>
              <path d="M4 17v2a2 2 0 002 2h12a2 2 0 002-2v-2" stroke="#6ab04a" strokeWidth="2" strokeLinecap="round"/>
            </svg>
          </div>
          <div>
            <div style={{fontSize:14,color:"#c0d090",fontWeight:"bold"}}>Exportar Dados</div>
            <div style={{fontSize:11,color:"#3a5a2a"}}>Salva um arquivo .json com todos os registros</div>
          </div>
        </div>
        <button onClick={handleExport} style={{
          width:"100%",background:"linear-gradient(135deg,#2a6a0a,#1a4006)",
          color:"#c0f090",border:"1px solid #3a8a1a",
          padding:"13px",borderRadius:10,fontSize:13,letterSpacing:1,
          textTransform:"uppercase",cursor:"pointer",fontFamily:"inherit",fontWeight:"bold",
          boxShadow:"0 4px 16px rgba(42,106,10,0.3)",
        }}>
          ↓ Exportar agora
        </button>
        <div style={{fontSize:10,color:"#2a4a2a",marginTop:8,textAlign:"center"}}>
          Salve o arquivo no iCloud, WhatsApp ou Google Drive para transferir entre dispositivos
        </div>
      </div>
      ...
      <div style={{
        ...
        <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:10}}>
          <div style={{
            ...
            <svg width="20" height="20" viewBox="0 0 24 24" fill="none">
              <path d="M12 22V9M8 13l4-4 4 4" stroke="#4ab090" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/>
              <path d="M4 17v2a2 2 0 002 2h12a2 2 0 002-2v-2" stroke="#4ab090" strokeWidth="2" strokeLinecap="round"/>
            </svg>
          </div>
          <div>
            <div style={{fontSize:14,color:"#c0d090",fontWeight:"bold"}}>Importar Dados</div>
            <div style={{fontSize:11,color:"#3a5a2a"}}>Carrega registros de um arquivo .json exportado</div>
          </div>
        </div>
        <input
          ref={fileRef}
        ...
        <button onClick={()=>fileRef.current?.click()} style={{
          width:"100%",background:"linear-gradient(135deg,#0a3a2a,#061a14)",
          color:"#80d0b0",border:"1px solid #1a6a4a",
          padding:"13px",borderRadius:10,fontSize:13,letterSpacing:1,
          textTransform:"uppercase",cursor:"pointer",fontFamily:"inherit",fontWeight:"bold",
          boxShadow:"0 4px 16px rgba(10,90,58,0.3)",
        }}>
          ↑ Selecionar arquivo
        </button>
        {importError && (
          <div style={{
            ...
            background:"#2a0d0d",border:"1px solid #6a2a2a",
      ...
      <div style={{
        background:"linear-gradient(145deg,#0f1e0a,#080e06)",
        border:"1px solid #152010",borderRadius:14,padding:"16px",
        boxShadow:"0 4px 16px rgba(0,0,0,0.4)",
      }}>
        <div style={{fontSize:9,letterSpacing:2,textTransform:"uppercase",color:"#3a6a2a",marginBottom:12}}>
          Como transferir entre celulares
        </div>
        {[
          {n:"1",txt:"No celular antigo, toque em Exportar agora"},
          {n:"2",txt:"Salve o arquivo no WhatsApp (mande para si mesmo) ou iCloud Drive"},
          {n:"3",txt:"No celular novo, abra o app e vá em Configurações"},
          {n:"4",txt:"Toque em Selecionar arquivo e abra o arquivo exportado"},
          {n:"5",txt:"Pronto — todos os dados aparecem no novo celular"},
        ].map(s=>(
          <div key={s.n} style={{display:"flex",gap:10,marginBottom:10,alignItems:"flex-start"}}>
            <div style={{
              width:22,height:22,borderRadius:"50%",flexShrink:0,
              background:"linear-gradient(135deg,#2a6a0a,#1a4006)",
              display:"flex",alignItems:"center",justifyContent:"center",
              fontSize:11,fontWeight:"bold",color:"#a0d060",
            }}>{s.n}</div>
            <div style={{fontSize:12,color:"#5a8a5a",lineHeight:1.5,paddingTop:2}}>{s.txt}</div>
          </div>
        ))}
      </div>
    </div>
...
export default function App() {
  ...
  const TABS=[
    ...
    {id:"config",label:"Config",Icon:({size=21,active=false})=>(
      <svg width={size} height={size} viewBox="0 0 24 24" fill="none">
        <circle cx="12" cy="12" r="3" stroke={active?"#e8c84a":"#3a5a2a"} strokeWidth="1.5"/>
        <path d="M12 2v2M12 20v2M4.22 4.22l1.42 1.42M18.36 18.36l1.42 1.42M2 12h2M20 12h2M4.22 19.78l1.42-1.42M18.36 5.64l1.42-1.42" stroke={active?"#e8c84a":"#3a5a2a"} strokeWidth="1.5" strokeLinecap="round"/>
      </svg>
    )},
  ];
  ...
  return (
    <div style={{
      ...
      <div style={{flex:1,overflowY:"scroll",overflowX:"hidden",WebkitOverflowScrolling:"touch",minHeight:0,position:"relative",zIndex:5,padding:"14px 14px 0"}}>
        ...
        {aba==="config"    && <Config registros={registros} onImport={data=>{setRegistros(()=>data);toast("Dados importados com sucesso!");setAba("resumo");}}/>}
      </div>
